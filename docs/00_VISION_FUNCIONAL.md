...
**Reportado por el dueño del proyecto (15/09).** El Tutor IA estaba derivando hacia un
comportamiento de "chat genérico" en vez de sostener su esencia como acompañante pedagógico. La
visión, en sus propias palabras, resumida aquí para que quede escrita y no en el aire (documento
**vivo**: el equipo está abierto a corregirla, no es una especificación cerrada):

1. **No es un chat, es un Acompañante.** No es el profesor que da respuestas — es un asistente que
   ayuda a superar bloqueos, con una lógica intuitiva tipo Duolingo o Anki: el estudiante entra, se
   autoevalúa, y el sistema le propone el siguiente reto adecuado.
2. **Gestión del Mastery y peso proporcional al esfuerzo.** No se busca que el estudiante suba de
   dominio rápido con ejercicios triviales — repetir un ejercicio sencillo debe pesar menos en el
   cálculo de Mastery que resolver uno difícil al primer intento. El avance debe ser proporcional al
   esfuerzo y la complejidad, no a la cantidad de clics.
3. **Repetición espaciada como motor central**, sugiriendo repasos según cuánto tiempo pasó desde
   la última vez que el estudiante demostró dominio real (ya cubierto por el Pilar 2, §2.2 — no
   estaba en discusión, se reafirma).
4. **Flexibilidad pedagógica para el docente**: crear cursos, organizar unidades y cargar bancos de
   actividades sin complicaciones técnicas — el contenido es el rey, la gestión debe ser
   transparente.
5. **Comportamiento esperado del Tutor**: si el estudiante tiene dudas, debe poder proponerle un
   ejercicio concreto del banco de actividades, no solo texto; debe "sentir" si está listo para
   avanzar o si necesita reforzar un punto débil; debe existir control de pesos reales entre
   actividades evaluativas (examen) y de práctica formativa — no solo un número que alguien tipea a
   mano por actividad.

**Contraste contra el código real, el mismo día (antes de cambiar nada):**

| Punto de la visión | Estado real encontrado |
|---|---|
| Acompañante que propone ejercicios | **No existía ningún camino de código** entre la conversación del Tutor (`TutorService.sendMessage()`) y el banco de `Activity`/`ActivityQuestion` — el único freno contra el "chat genérico" era una instrucción de texto en el prompt, sin ninguna barrera estructural. |
| Mastery proporcional al esfuerzo | `mastery.calculator.ts` tomaba `Math.max(...scores)` (el mejor score histórico) sin mirar cuántos intentos tomó ni la dificultad de la actividad — repetir un ejercicio `BASICO` 10 veces pesaba igual que acertarlo a la primera. |
| Peso real examen vs. práctica | En **todo** el sistema existe un solo `ActivityType` (`AUTO-EVAL`, `baseWeight: 1.0`, `src/seeds/seed-runner.ts`). El ejemplo de tres categorías con peso distinto que ya documenta `03_MOTOR_Y_TUTOR.md` §3.2 (Quiz=1.0, Taller=1.5, Parcial=3.0) es un ejemplo aspiracional que **nunca se implementó como categorías reales** — el formulario de crear actividad (`docente/ejercicios/crear.vue`) ni siquiera tenía un selector visible, se asignaba en silencio al primer tipo de la lista. |
| Docente puede editar/gestionar contenido | Razonablemente cerca — CRUD completo ya existe y la UI de edición se cerró el 15/09 (Fase 16 de Antigravity), fuera del alcance de esta pausa. |
| Repetición espaciada (SM-2) | Sin divergencia real — ya funciona como lo describe el Pilar 2. |

**Qué se implementó el mismo checkpoint (Claude Code, verificado en navegador real, no solo en
tests):**

- **`src/common/utils/mastery.calculator.ts`**: una actividad `Difficulty.BASICO` repetida más de 2
  veces empieza a descontar el score que cuenta para el Mastery (piso del 50%, nunca llega a cero).
  Una actividad `INTERMEDIO`/`AVANZADO` repetida no se penaliza — ahí sí hay aprendizaje real en el
  intento, no "grindeo".
- **`TutorRecommendationService`** (nuevo): convierte repasos vencidos (`review-schedules`) y
  mastery por unidad en una sugerencia concreta de actividad. El Tutor ahora habla primero al
  abrirse (`GET /tutor/greeting`), citando datos reales ("tienes pendiente repasar X desde hace N
  días"), y detecta cuando el estudiante pide explícitamente practicar ("quiero practicar...") para
  ofrecerle una actividad concreta con una tarjeta clicable en el chat.
- **Primer diseño (marcador oculto `[[LISTO_PARA_PRACTICAR]]` que el LLM escribía y el backend
  interpretaba) se descartó tras feedback del dueño del proyecto** por sentirse "muy evaluativo" —
  como un formulario oculto, no una conversación — y por costo de tokens en cada llamada. Se
  reemplazó por una instrucción ligera en el prompt: el Tutor puede ofrecer practicar o repasar
  contenido **con sus propias palabras**, cuando de verdad aporte, sin que el backend fuerce nada.
  Por el mismo motivo (no saturar al estudiante ni al LLM) se quitó también la tarjeta automática
  que se adjuntaba en *cada* respuesta dentro de una unidad de mastery bajo — ahora la tarjeta solo
  aparece en el saludo proactivo o cuando el estudiante la pide explícitamente.

**Qué queda abierto, declarado como sugerencia — no decidido todavía:**

- **Categorías reales de `ActivityType`** (ej. `QUIZ`/`TALLER`/`PARCIAL` con `baseWeight` 1.0/1.5/3.0,
  igual que ya documenta `03_MOTOR_Y_TUTOR.md` §3.2) — implementaría de verdad el punto 5 de la
  visión de arriba. No se construyó en este checkpoint porque no se confirmó explícitamente; queda
  como candidato evidente para una siguiente fase.
- **"Ir al contenido de la unidad" como alternativa a "practicar un ejercicio"** — hoy el Tutor solo
  lo ofrece como texto conversacional (el LLM lo dice con palabras), sin un mecanismo estructurado
  como el que sí existe para "practicar". Si en el uso real se vuelve un patrón frecuente, vale la
  pena construirle su propio contrato de datos.


### 9.6 Checkpoint 2026-09-19 — Cierre del Tutor: decisiones del dueño del proyecto

Una auditoría del Tutor contra el código real (19/09) dejó ver que la lógica pedagógica (saludo proactivo, sugerencia de ejercicio, decaimiento de Mastery) funciona, y que lo abierto era la experiencia de chat, la robustez y algunas piezas de la visión. El dueño del proyecto tomó estas decisiones:

| Tema | Decisión | Estado |
|---|---|---|
| Proveedor y clave | Solo Google Gemini, con la **clave gratuita de Google AI Studio de cada estudiante** (se pide en el registro con opción de omitir, o al primer uso del Tutor, con los pasos para conseguirla). Se retira OpenAI y la respuesta simulada de reserva. | Backend ✅ (ADR 10). Interfaz: `docs/_archivo/PLAN_IMPLEMENTACION_ANTIGRAVITY_2026-09-20.md` §19. |
| Alcance del "100% del Tutor" | Incluye: botón estructurado **"Ir al contenido"** de la unidad, **repasos vencidos dentro del chat** (no solo en el saludo), **configuración del Tutor por el docente** (activar/desactivar, tope de ayuda y estilo) y una **barrera en código** contra respuestas que den la solución completa (ver las filas siguientes; ambas ya tienen backend). | Pendiente de diseño y de plan — ver `docs/PLAN_MAESTRO.md` §6.4. |
| "Nivel de guía" (chips 1. Pista / 2. Pregunta / 3. Falla) | **Automático y visible.** Vienen del principio P03 (andamiaje progresivo: pista conceptual → pregunta guía → localización de la falla, sin entregar código; base: VanLehn 2011, Koedinger y Aleven 2007). El sistema lo decide con los intentos fallidos del estudiante en la actividad (0–1 → nivel 1; 2–3 → nivel 2; 4 o más → nivel 3), se lo indica al LLM y la interfaz solo lo muestra; el estudiante no lo elige. Los umbrales son una propuesta inicial ajustable. | Backend ✅ (19/09). Interfaz: `docs/_archivo/PLAN_IMPLEMENTACION_ANTIGRAVITY_2026-09-20.md` §18.4. |
| Configuración del Tutor por el docente | En **tres ámbitos — clase, unidad y actividad** (el más específico manda; cada docente decide qué quiere lograr en cada unidad y cómo necesita que el estudiante desarrolle cada actividad): **activar o desactivar**, **tope de ayuda** y **estilo** entre cuatro opciones fijas (sin texto libre, para no abrir una vía de inyección de prompt). En el chat general se aplica lo más estricto entre las clases activas del estudiante. | Backend ✅ (19/09, ADR 11). Interfaz: `docs/_archivo/PLAN_IMPLEMENTACION_ANTIGRAVITY_2026-09-20.md` §20. |
| Barrera contra dar la solución | **Tope de código inteligente, sin llamadas extra al LLM.** Refinado por el dueño: un tope fijo de pocas líneas bloquearía explicaciones legítimas de quien quiere aprender, así que solo actúa **dentro de una actividad** y solo omite un bloque si es un volcado larguísimo (más de 20 líneas) o tiene la forma de la solución de un ejercicio (programa que lee la entrada y la imprime), salvo que se esté explicando el propio código del estudiante. Es una heurística, no una garantía (ADR 11). | Backend ✅ (19/09). |
| Experiencia del chat (errores legibles, accesibilidad, scroll, Markdown, espera, móvil, historial visible) | Se delega a Antigravity. | `docs/_archivo/PLAN_IMPLEMENTACION_ANTIGRAVITY_2026-09-20.md` §18. |
