...
| Repasos — «Iniciar Refuerzo» (`/estudiante/repasos`) | ❌ | **Hallado por el dueño y confirmado contra el código el 21/09:** `completeReview` solo quita el repaso de la lista en memoria (no navega ni escribe nada) y vuelve al recargar; además el mapeo del store descarta `learningUnitId`. El backend ya reprograma solo (SM-2 al calificarse una actividad de la unidad). Corrección: Fase 23, T1. |
| Roles de usuario: cambio por el admin, registro con elección de rol y solicitud de docente con aprobación | ⚠️ backend ✅ / interfaz pendiente | **Backend hecho y probado por Claude Code, 21/09.** Antes: `PATCH /users/:id/role` aceptaba cualquier texto y un admin podía quitarse su propio rol; el registro no ofrecía rol (todos estudiantes, correcto). Ahora: el rol se valida (`400` si no es admin/docente/estudiante), un admin no puede cambiar el rol, desactivar ni eliminar su propia cuenta (`403`); `POST /auth/register` acepta `requestedRole` y `roleRequestReason` opcionales — la cuenta **siempre** nace estudiante y pedir docente crea una solicitud `pending` (tabla `role_requests`, migración `1789400000000`); solo un admin la aprueba o rechaza (`GET/PATCH /role-requests`, `GET /role-requests/me`), con quién y cuándo — cualquier correo puede pedirlo, por decisión explícita del dueño (21/09: quitó la restricción de correo institucional que existía en un primer borrador). El rol se lee de la base en cada petición, así que aprobar surte efecto sin reiniciar sesión. **Interfaz pendiente:** Fase 23, T2–T4. **Falta para producción:** una forma segura de crear el primer admin (hoy solo existe la cuenta demo con contraseña conocida en el seed). |
...
| Primer admin real (sin depender de la cuenta demo con contraseña pública) | ✅ | `npm run admin:create-first` (`src/scripts/create-first-admin.ts`, 22/09) — crea un admin desde `ADMIN_EMAIL`/`ADMIN_PASSWORD`/`ADMIN_FULL_NAME` del entorno del propio servidor, con la misma política de contraseña que el registro público; idempotente por email (no crea ni modifica si ya existe, para no pisar una cuenta real en una segunda corrida). Se corre una vez al desplegar. Tests: `create-first-admin.spec.ts` |
| Artefactos de despliegue (Dockerfile, compose de producción, Caddy, respaldos, guía) | ✅ | `docs/DESPLIEGUE.md`, ADR 12. **Verificado con Docker real (23/09):** imagen compilada, arranque en producción contra MariaDB 11.4 (9 migraciones + seed), `/health`, Swagger apagado, sandbox y `child_process` bloqueado dentro del contenedor, migraciones y primer admin desde la imagen. Pendiente: probarlo en la máquina real (Oracle/Vercel) — lo hace el dueño con sus cuentas |
| Recuperación de contraseña por correo | ✅ | `POST /auth/forgot-password` y `/auth/reset-password` (enlace de 30 min, un solo uso, solo se guarda el hash; cambiar la contraseña cierra las sesiones anteriores), SMTP configurable, pantallas `/auth/forgot-password` y `/auth/reset-password`. Probado con un servidor SMTP de prueba y en Chrome real |
| Ejecución del despliegue | ❌ | Tarea de esta semana, `S06-J03` (Jeider) — crear la cuenta elegida, configurar variables de entorno (incluido `TUTOR_KEY_ENCRYPTION_SECRET` y, al final, `ADMIN_EMAIL`/`ADMIN_PASSWORD` para `admin:create-first`), desplegar backend real y verificar login contra el entorno desplegado. Autorización OAuth del conector de Vercel sigue pendiente, ahora acotada solo al frontend |
...
| 2026-09-21 | **Fase 22 de Antigravity auditada y archivada; backend y clave del Tutor verificados; carpeta y guardia para el rediseño visual de José.** (1) **Fase 22, auditoría en navegador real:** T1, T3 y T4 correctos; **T2 tenía un defecto** que el informe da por resuelto (sin clave guardada el foco caía al `<body>` y `Escape` no cerraba el panel del Tutor), corregido por Claude Code (`Escape` escuchado en el documento y foco reasignado cuando cambia el panel de la clave); el diálogo de `sistema.vue` estaba bien. (2) **Backend:** `POST /maintenance/cleanup` ya no informa éxito si falla y devuelve los conteos; **`GET /enrollment/my` devolvía también matrículas retiradas y pendientes** (su descripción dice «activas»; la interfaz no filtra por estado y las mostraba como clases cuyo contenido respondía 403): ahora solo `active`. Ambos con tests. (3) **Clave del Tutor con Gemini real, recorrido completo desde la interfaz** (detalle en la fila del Tutor de §4.1): faltaba `TUTOR_KEY_ENCRYPTION_SECRET` en el `.env` local y el error se presentaba como reintentable; corregido. (4) **Identidad visual (José):** `docs/identidad-visual/`, `npm run check:identidad` y `FLUJO_GIT.md` actualizado (§6.1.1). (5) Fase 22 archivada; el plan vigente de Antigravity queda sin fase pendiente. **Autocorrección del auditor:** mi primera prueba de «Tutor desactivado» dio un falso positivo porque la matrícula demo de Pedro en la clase 14 está en `withdrawn` desde la noche del 20/09 (quedó de una prueba anterior) y desactivar esa clase no lo afecta; repetida con la clase 15 dio el resultado correcto. **No se restauró esa matrícula:** el clasificador denegó la escritura en la base, decide el dueño. **Pendiente conocido, no corregido:** al unirse a una clase que exige aprobación, la pantalla dice «¡Te has matriculado exitosamente!» aunque la matrícula quede pendiente. (6) **José subió a `main` un primer avance visual** mientras se cerraba la ola (`7e9f314`, `fc9b5bb`): integrado con merge sin conflictos y revisado (§6.1.1); trae dependencias y lógica nuevas y un panel docente con ceros inventados. `verify:clean` y la suite completa: ver `CHANGELOG.md`. | Claude Code (Sonnet 5) |
| 2026-09-21 (2ª pasada) | **Roles y registro con aprobación, Fase 23, guía de QA de la semana y ayudas para José.** A pedido del dueño, que reportó tres fallas y una petición: (1) **«Iniciar Refuerzo» no lleva a ninguna parte** — confirmado contra el código (botón que solo quita el repaso de la lista). (2) **El admin no puede editar roles** — el endpoint existía pero sin validar; ahora valida y protege la propia cuenta, y falta la interfaz. (3) **Registro con elección de rol y aprobación del admin** — diseñado e implementado en el backend (§4, fila de roles): el registro siempre crea un estudiante, pedir docente deja una solicitud pendiente que solo un admin resuelve, sin restricción de dominio de correo. Prácticas que se aplicaron y que se le explicaron al dueño: mínimo privilegio, el cliente nunca envía el rol, aprobación humana con constancia (quién y cuándo), validación de entrada, límite de peticiones y sin escalada por el propio usuario. **Se descartó, por decisión explícita del dueño, restringir qué correos pueden pedir el rol docente** (existió brevemente como `TEACHER_EMAIL_DOMAINS`, ya retirado) — verificación de correo por enlace queda como mejora futura, no como requisito. Suite de tests del backend ampliada; `verify:clean` repetido tras la migración nueva (ver `CHANGELOG.md`). (4) **Fase 23** para Antigravity (repasos, rol en `/admin`, registro, solicitudes). (5) **Guía de auditoría de Jorge** para la semana del 21 al 25 (`docs/ReportesQA/GUIA_AUDITORIA_2026-09-21.md`), con pruebas adversariales de roles. (6) **José:** bitácora, guía de UX/UI y reglas de trabajo en paralelo. **Veredicto de despliegue, verificado el 21/09:** el código está listo para desplegarse en lo técnico (`verify:clean` en verde, endurecimiento hecho, Tutor con clave real probado). **No se recomienda desplegar todavía** por: las tres fallas funcionales anteriores (Fase 23), el primer administrador (solo existe la cuenta demo con contraseña pública), el panel docente con ceros inventados, y la ejecución en sí (`S06-J03`: cuenta en la plataforma, `TUTOR_KEY_ENCRYPTION_SECRET` en el servidor). | Claude Code (Sonnet 5) |
| 2026-09-22 | **Fase 23 auditada, fusionada a `main`, y primer admin real resuelto — a pedido del dueño ("¿ya estamos listos para desplegar?").** (1) **Fase 23 verificada contra código real** (no solo contra el informe de Antigravity): las 4 tareas coinciden con el plan y el contrato del backend (detalle en §6.1). Build limpio, suite completa del backend en verde (61/61 suites, 539/539 tests tras el agregado del punto 3), `npx nuxi typecheck` en 0 errores. **Squash-merge a `main`** (`0397d02`) con confirmación explícita del dueño; rama `feat/fase-23` borrada (local y remota) tras confirmar que el diff fusionado es idéntico al de la rama. (2) **La falla suelta `validate-pre-frontend.spec.ts`** que había quedado sin explicar en la sesión anterior se investigó: pasa en aislado (13/13) y pasa dentro de la corrida completa repetida — mismo patrón de flakiness bajo carga que el worker del judge, no una regresión real, sin cambio de código. (3) **Primer admin real:** el dueño eligió, entre las opciones planteadas, un script por variable de entorno en vez de solo cambiarle la contraseña a la cuenta demo. Se agregó `npm run admin:create-first` (`src/scripts/create-first-admin.ts`) — lee `ADMIN_EMAIL`/`ADMIN_PASSWORD`/`ADMIN_FULL_NAME` del entorno real del servidor, exige la misma política de contraseña que el registro público, e idempotente por email (no pisa una cuenta si ya existe). Con test propio (`create-first-admin.spec.ts`, 3 casos) — se detectó y corrigió en la misma sesión un efecto secundario real: sin la guarda `require.main === module`, el solo hecho de importar el archivo para probarlo ejecutaba `bootstrap()` y dejaba `process.exitCode = 1` en cualquier corrida de `npm test`. §4.7 actualizado. **Pendientes que siguen abiertos, sin cambios:** el panel docente con `atRiskCount` fijo en 0 (`pages/docente/index.vue:590`, comentario propio "placeholder — requiere datos del backend"; territorio de José, fuera de esta pasada) y el reporte de la auditoría de Jorge de la semana (`GUIA_AUDITORIA_2026-09-21.md` sigue sin resultados, entrega prevista el 25/09). | Claude Code (Sonnet 5) |

| 2026-09-23 | **Simulación de usuario, verificación del reporte de Jorge y respuesta a «¿ya se pueden usar todas las funciones?».** Chrome real contra una base desechable como estudiante, docente y admin; barrida de las 104 operaciones por rol. El reporte de Jorge (23/09) auditó `ef88916` y su veredicto no cubre roles, Fase 23, mensajería ni notificaciones; sus citas de código son correctas. Corregido con tests: `join` sin código matriculaba en la primera clase, el código de ingreso se filtraba, `activity-log`/`activities`/`users`/`message` sin control de relación, módulos sin publicar visibles al estudiante, PATCH que movía contenido a otra clase, nota de código sin escalar a los puntos, KPI y textos inventados, móvil, autoguardado, `ai_evaluated`. **El estudiante ya puede usar todas sus funciones** (probados los 6 tipos de actividad, Tutor con clave inválida, mensajes, notificaciones, repasos, unirse a clase). **El docente todavía no puede armar un curso de cero desde la interfaz ni crear ejercicios que no sean de código, el admin no puede registrar/desactivar usuarios y nadie tiene pantalla de perfil** → Fase 24. Detalle de hallazgos en un documento privado (`.gitignore`). | Claude Code (Sonnet 5) |
| 2026-09-25 | **Fase 25 cerrada, Fase 26 escrita y pendientes cerrados.** Fase 25: Parte B de Antigravity auditada en Chrome real (móvil inutilizable y «Probar» silencioso, corregidos), A7 hecha y una vía de inyección anterior en el renderizador de Markdown cerrada con DOMPurify. Fase 26 (editor CodeMirror 6, resaltado y selector de lenguaje) escrita para Antigravity; su Parte A de backend ya entregada (`coding` solo acepta `javascript`; `fill_code` acepta `language` de resaltado). Pendientes propios cerrados: errores del servidor visibles en 11 páginas antiguas, F24-09 (`PATCH /class` 403 y código inmutable), F24-10 (cambiar la clave cierra las otras sesiones y entrega un token nuevo), Tutor en ejercicios de HTML y CSS (ahora ve el HTML/CSS del estudiante y la barrera anti-solución cubre páginas completas), `verify:clean` con credenciales por defecto en las dos mitades, y `DESPLIEGUE.md`/ADR 13 sin *Pay As You Go* (decisión del dueño: ningún riesgo de cobro). Detalle y evidencia en `CHANGELOG.md`. |
---
...
**HTML/CSS/JS entra en el alcance** (el curso lo exige). Hoja de ruta: **Fase 25** (plan escrito el 24/09 en `docs/antigravity/PLAN_IMPLEMENTACION.md`; **Fase 25 CERRADA el 25/09: Parte A backend (24/09, `verify:clean` en verde), Parte B de Antigravity auditada y corregida, y A7 (código de las lecciones conservado + defensa DOMPurify en el navegador); 73 suites / 702 tests, ver `CHANGELOG.md`.** **Parte A backend de Claude Code** — `html_css`, migración, evaluador con jsdom, vista del estudiante, «probar» sin gastar intento, solución modelo obligatoria y arreglo del saneado de bloques de código — y **Parte B frontend de Antigravity** — constructor de reglas, editor + vista previa en `iframe sandbox=""` + reglas), 5 a 6 días; **Fase 26** — editor con resaltado (CodeMirror 6 bajo demanda) y selector de lenguaje, 3 a 4 días; **Python** opcional después del MVP con Pyodide dentro del sandbox actual (3 a 5 días); **C, C++ y Java se sacrifican** en la versión gratuita. **Fase 26 escrita el 25/09** en `docs/antigravity/PLAN_IMPLEMENTACION.md` (Parte A backend entregada: `coding` solo acepta `javascript`, `fill_code` acepta `language` de resaltado; Parte B frontend, de Antigravity, pendiente).

### 6.1 Antigravity — Fase 24 entregada, auditada, corregida y fusionada a `main` (24/09); Fases 21, 22 y 23 entregadas y auditadas; Fases 18 a 20 con auditoría parcial

**Fase 24 — entregada por Antigravity (`feat/fase-24`, informe `INFORME_2026-09-24_SESION_01.md`), auditada por Claude Code el 24/09 (`docs/ReportesQA/REPORTE_AUDITORIA_QA_FASE24_2026-09-23.md`: el creador de ejercicios no enviaba, reordenar lecciones no funcionaba, los errores del servidor no se veían y se había perdido «¿Olvidaste tu clave?») y corregida el mismo día (24b, ver `CHANGELOG.md`; 25/25 comprobaciones en Chrome real)..** Fusionada a `main` el 24/09 (`4ca0301`). Escrita el 23/09 en `docs/antigravity/PLAN_IMPLEMENTACION.md` con los contratos del backend verificados: **T1** constructor del curso (módulos, temas, unidades y lecciones; hoy una clase creada desde la interfaz queda vacía), **T2** creador de ejercicios de los 6 tipos (hoy solo código), **T5** gestión de la clase, **T4** perfil y contraseña, **T3** administración de usuarios (registrar, desactivar, restablecer contraseña). Rama `feat/fase-24`, sin ejecutar.

**Fase 23: entregada por Antigravity (commits `d8187b8`, `ccc072f`, `435dd47`, `3d52bef`, informe `INFORME_2026-09-21_SESION_02.md`) y auditada por Claude Code el 22/09.** Verificado contra el código real, no solo contra el informe: `pages/estudiante/repasos.vue` navega a `/estudiante/unidad/:id` y ya no existe `completeReview`; `pages/admin/index.vue` llama `PATCH /users/:id/role`, bloquea la fila del propio admin y muestra el 403 real del servidor; `pages/auth/register.vue` solo deja elegir `estudiante`/`docente` (nunca `admin`) y no quedó ningún rastro de restricción de dominio de correo; el panel de solicitudes y el aviso al estudiante llaman `GET/PATCH /role-requests`. Diff acotado a `frontend-nuxt/` (9 archivos, sin tocar estilos globales ni backend). **Suite completa del backend: 60/60 → 61/61 tras un agregado propio, 536/536 → 539/539 tests; build y `npx nuxi typecheck` en 0 errores.** Fusionada a `main` (squash, commit `0397d02`) con confirmación explícita del dueño del proyecto. Una falla suelta que apareció en la corrida completa anterior (`validate-pre-frontend.spec.ts`) se investigó y es el mismo patrón de flakiness bajo carga ya documentado con el worker del judge — pasa en aislado y dentro de la corrida completa repetida, no es una regresión de código.
...
No es un plan de implementación: José decide el diseño y su carpeta (`docs/identidad-visual/`) solo fija el territorio y el orden. Contiene una **guía de trabajo** (qué es libre, qué se avisa y qué no se toca; ramas por etapa: tokens → componentes → páginas; cómo probar y entregar), los **primeros pasos** (las 14 decisiones globales que hay que tomar antes de tocar código para que toda la página quede coherente) y una **plantilla de propuesta** que José llena. Con esa propuesta, Claude Code revisa el contraste, traduce los valores a `tailwind.config.ts` y ajusta la guía. Protección: `npm run check:identidad` compara la rama contra `main` y falla si toca algo fuera del estilo y las plantillas de `frontend-nuxt/`; avisa si cambia un `<script>`, pierde una directiva de comportamiento o un `id`/`aria-label`. Hallazgos que ya se le entregan: el estilo sale casi todo de `tailwind.config.ts` (solo 4 de 37 archivos `.vue` tienen colores escritos a mano, 32 usos), los iconos son emojis en 28 archivos y `lucide-vue-next` está instalado pero sin uso. Además de la guía tiene una **bitácora propia** (`BITACORA.md`: una fila por cambio con su commit, y la lista de «archivos en obra» para no chocar con lo funcional), una guía de **UX/UI más allá de la paleta** (`02_MEJORAR_UX_UI.md`, con errores reales del proyecto) y la sección «Trabajar en paralelo sin conflictos» del README. Se recomendó unir sus ramas con «Create a merge commit» (no Squash) para conservar un commit por pantalla y poder deshacer una sola. **Estado:** José subió un primer avance directo a `main` (`7e9f314`: paleta `stire-*`, `HeaderNav` y `DOC-V01`); se integró con merge y se revisó el 21/09 (`typecheck` en 0 tras `npm ci`). Siete observaciones en `docs/identidad-visual/README.md` §5, las de más peso: cambió comportamiento (búsqueda, modal de QR con la dependencia nueva `qrcode`, menú móvil), **las tarjetas del panel docente muestran ceros inventados** (`atRiskCount = 0` fijo; el backend no devuelve `enrollmentCount` ni `avgMastery` en `/class/my-classes`), `src/index.css` usa sintaxis de Tailwind 4 y no se carga (el proyecto es Tailwind 3), y hay 46 clases de color por defecto que no siguen a los tokens. **Sin corregir a propósito:** es su archivo en curso y editarlo aquí provocaría conflicto; se le devuelve con el detalle. Aún no entrega la propuesta.
