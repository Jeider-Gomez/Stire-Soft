...
**Vigente: Fase 23**, escrita el 21/09 en `docs/antigravity/PLAN_IMPLEMENTACION.md` (solo lo pendiente): **T1** «Iniciar Refuerzo» que lleve a la unidad y deje de simular que el repaso terminó; **T2** control para que el admin cambie el rol de un usuario; **T3** elección estudiante/docente en el registro (con motivo opcional); **T4** panel del admin con las solicitudes de docente (aprobar/rechazar) y aviso de estado al solicitante. Contratos exactos del backend incluidos. Rama `feat/fase-23`, sin editar estilos (José). Sin ejecutar.
