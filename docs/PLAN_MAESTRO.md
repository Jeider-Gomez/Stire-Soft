...
### 6.1 Antigravity — Fases 18 y 19 pendientes (Tutor IA); Fase 17 cerrada

**Pendiente, escrita el 19/09** (`docs/antigravity/PLAN_IMPLEMENTACION.md` §18, §19 y §20), salida de una
auditoría del Tutor contra el código real: **§18** — experiencia del chat (errores legibles según el
contrato real de códigos HTTP, scroll, accesibilidad del drawer, Markdown con bloques de código, espera
escalonada, móvil, historial visible tras recargar); **§19** — interfaz de la clave gratuita de Google AI
Studio de cada estudiante (campo opcional con "Omitir" en el registro, panel con los pasos dentro del
chat, gestión de la clave); **§20** — panel del docente para configurar el Tutor por clase, unidad y actividad (y el estado "desactivado por tu docente" del lado del estudiante). El "Nivel de guía" quedó decidido como automático y visible (§18.4): el backend ya lo
calcula y la interfaz solo lo muestra. Ninguna está ejecutada ni auditada todavía.
