...
✅ **Auditada en FASE CC-06, Paso 2** (antes: declarada pero no verificada). Ratios reales
(fórmula WCAG de luminancia relativa) contra la paleta final de `Color/` en Figma (fileKey
`1MjKiDrjU65ezO3ztO0v4m`):

| Par | Hex | Ratio | Requisito | Resultado |
|---|---|---|---|---|
| Texto primario / bg primario | `#2B2622` / `#F6F3EF` | 13.53:1 | ≥4.5:1 | ✅ |
| Texto primario / blanco | `#2B2622` / `#FFFFFF` | 14.97:1 | ≥4.5:1 | ✅ |
| Texto secundario / bg primario | `#6F6761` / `#F6F3EF` | 5.01:1 | ≥4.5:1 | ✅ (ajustado — ver abajo) |
| Texto secundario / bg secundario | `#6F6761` / `#EDE8E1` | 4.55:1 | ≥4.5:1 | ✅ (ajustado) |
| Blanco / ámbar-fuerte (botón/chip) | `#FFFFFF` / `#A76719` | 4.57:1 | ≥4.5:1 | ✅ (ajustado — ver abajo) |
| Blanco / pasa, falla, info, vencido | — | 5.04 – 6.92:1 | ≥4.5:1 | ✅ sin ajuste |
| Ámbar (marca) / bg, blanco (no-texto, ≥24px) | `#C87B1E` | 3.01 – 3.33:1 | ≥3:1 | ✅ sin ajuste |
| Borde funcional / bg, blanco | `#998878` | 3.09 – 3.42:1 | ≥3:1 | ✅ (nuevo token — ver abajo) |

**Dos colores fallaron la medición inicial y se ajustaron** (no se declaró cumplimiento sin medir):
1. **Texto secundario** `#7A716A` → **`#6F6761`**: el original daba 4.32:1 contra el fondo primario
   (falla 4.5:1). Se oscureció manteniendo el matiz cálido hasta pasar contra los tres fondos donde
   se usa (bg primario, bg secundario, blanco).
2. **Ámbar de marca `#C87B1E` como texto/relleno de botón con texto blanco**: solo 3.33:1 (falla
   4.5:1 para texto). Se creó un segundo token, **`color/acento/ambar-fuerte` = `#A76719`**,
   exclusivamente para texto/chips sobre relleno ámbar — el `#C87B1E` original se conserva para
   íconos y superficies grandes (≥3:1, donde sí pasa).
3. **Borde decorativo `#C9C1B8`** no alcanza 3:1 como borde funcional (1.61–1.78:1) — se mantiene
   solo para divisores decorativos (sin requisito WCAG) y se agregó **`color/base/borde-fuerte` =
   `#998878`** (3.09–3.42:1) para bordes que sí delimitan un componente interactivo (inputs, foco).

Detalle completo (24 pares medidos) en la ficha de vistas
[`3.3.1_FICHAS_VENTANAS.md`](3.3.1_FICHAS_VENTANAS.md) y en el archivo Figma, página "🎨 Fundaciones".
