...
| **F25-04** | Medio | Lecciones y enunciados con código | El servidor saneaba el código como HTML (codifica `<`→`&lt;`, borra `"<hola>"`, añade un `</b>`) y B0 ahora escapa otra vez: el código en línea con `<`, `>` o `&` se ve `a &lt; b &amp;&amp; c` | ⏳ **A7** (backend): dejar de sanear los segmentos de código; B0 ya está en `main`, así que es seguro hacerlo |
| F25-05 | Info | Plan §25.7, control 2 | «`err?.data?.message` debe dar cero» estaba mal planteado (**es del plan, no de Antigravity**): dan 15, todos **ya en `main`**, en páginas anteriores (mensajes, rendimiento, `docente/index`, `clases`, `[studentId]`, `useApi.ts`, `auth.ts:116`); muestran el texto genérico en vez del motivo del servidor | Pendiente (backlog, ninguna es de esta fase) |
...
**Sin hallazgos de seguridad.** Se comprobó también que el estudiante no recibe `modelSolution`, criterios ni reglas ocultas (Parte A, 25/25 por la API) y que la interfaz no las muestra.
