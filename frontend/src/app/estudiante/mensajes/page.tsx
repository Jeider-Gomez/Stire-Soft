  ...
  return (
    <div>
      <div className="page-header">
        <h1>💬 Mensajes</h1>
        <p>Comunicación con tu docente</p>
      </div>

      <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '1.5rem' }}>
        <button
          className={`btn ${tab === 'inbox' ? 'btn-primary' : 'btn-secondary'}`}
          onClick={() => { setTab('inbox'); setSelectedUserId(null); }}
        >
          📥 Recibidos
        </button>
        <button
          className={`btn ${tab === 'sent' ? 'btn-primary' : 'btn-secondary'}`}
          onClick={() => { setTab('sent'); setSelectedUserId(null); }}
        >
          📤 Enviados
        </button>
        <button className="btn btn-success" onClick={() => setShowCompose(true)}>
          ✏️ Nuevo Mensaje
        </button>
      </div>
      ...
      {selectedUserId && (
        <div className="card" style={{ marginBottom: '1.5rem' }}>
          <div className="card-header">
            ...
            <button className="btn btn-secondary btn-sm" onClick={() => setSelectedUserId(null)}>
              ← Volver
            </button>
          </div>
          <div style={{ maxHeight: '400px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '0.75rem', padding: '1rem 0' }}>
            {selectedConversation.map((msg: any) => (
              <div
                key={msg.id}
                className={`chat-message ${msg.senderId === user?.id ? 'user' : 'tutor'}`}
              >
          ...
          <div className="message-list">
            {messages.map((msg: any) => {
              ...
              return (
                <div
                  key={msg.id}
                  className={`message-item ${!msg.isRead && tab === 'inbox' ? 'unread' : ''}`}
